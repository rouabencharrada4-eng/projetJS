...
// Produits depuis la DB pour la catégorie Jeux
$stmt = $pdo->prepare('
    SELECT p.*, c.nom AS categorie
    FROM produits p
    JOIN categories c ON c.id = p.categorie_id
    WHERE c.slug = "jeux" AND p.actif = 1
...
function carteProduit($p, $wishlist_ids) {
    ...
    $is_extra    = str_starts_with((string)($p['id'] ?? ''), 'jeu-');
    ob_start();
    ?>
...
<section class="product-section" style="padding:50px 0;">
    <div class="container">
        ...
        <div class="page-banner">
            <h3>Jeux & Jouets</h3>
            <p>Amusez-vous avec notre sélection pour petits et grands</p>
        </div>
